        ...
        if($cordovaOauthUtility.isInAppBrowserInstalled()) {
          ...
          browserRef.addEventListener('loadstart', function(event) {
            if((event.url).indexOf(redirect_uri) === 0) {
              var url = event.url;
              alert(url);
              var code = /\?code=([0-9]+)\//.exec(url);
              var token = /\?code=[0-9]+\/([a-z0-9A-Z-]+)\&/.exec(url);
              var auth_user = /\&authuser=([0-9])\&/.exec(url);
              var session_state = /\&session\_state=(.+)\&/.exec(url);
              var prompt = /\&prompt=([a-zA-Z]+)\#/.exec(url);
              var error = /\?error=([0-9]+)\&?/.exec(url);
              if (code || error) {
                browserRef.close();
              }
